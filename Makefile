SHELL = /bin/bash
# One binary. The names metha used to install are symlinks to it: it reads the
# name it was invoked under and runs the matching verb, which is what keeps
# every existing script working. Separate binaries came to 186MB, because each
# one linked the whole program including the 11MB endpoint list; this is about
# 25MB, once.
TARGET = metha
LEGACY = metha-sync metha-cat metha-id metha-ls metha-files metha-fortune metha-stat metha-migrate
